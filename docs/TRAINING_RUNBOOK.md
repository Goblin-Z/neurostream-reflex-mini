...
| 支柱 | 数据 | 数量 | 教模型什么 |
|------|------|------|-----------|
| **逻辑提问** | `clarify_30k.jsonl`（新增 `scripts/generate_ask_data.py`） | 3 万 | 信息不足 → 主动问 1-3 个关键问题（具体、问号结尾、一次问全）→ 补充后完整回答 |
| **问答** | `sft_kd_clean.jsonl` 的 QA 部分（模板 QA + self-instruct） | 6 万 | 知识性问题 → 有条理的回答（分点/结构） |
| **多轮对话** | `sft_kd_clean.jsonl` 的 multi-turn 部分 | 4 万 | 承接上下文、追问（FOLLOWUPS）、引用前文 |
